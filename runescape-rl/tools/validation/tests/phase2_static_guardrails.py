...
def live_no_supplies_simplified_config() -> int:
    ...
    for path in paths:
        ...
        if parser.get("run", "action_version", fallback="").strip("'\"") != "fight_caves_multidiscrete_3_head_no_supplies_v4_run_energy_prayer8_stationary_attack_tick":
            failures.append(f"{label}: action_version is not the v4 run-energy contract")
        if parser.get("run", "observation_version", fallback="").strip("'\"") != "fight_caves_puffer_policy_obs_v9_run_energy_prayer_timing_mask8_no_supplies":
            failures.append(f"{label}: observation_version is not the v9 run-energy contract")
