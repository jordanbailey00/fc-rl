...
The strongest final checkpoint in our latest Fight Caves search completed **9,570 of 10,800 full caves: an 88.61% success rate**. It started at wave 1, survived the ordinary waves, and defeated Jad using a fixed Twisted bow loadout with no food or Prayer potions. Of the episodes that reached Jad, approximately **99.28%** ended in a kill.

Training that checkpoint took 499,122,176 agent steps and 434 seconds of training time on the experiment's RTX 5070 Ti, excluding final evaluation. Getting to that point took substantially more work: building the simulation, inspecting failed policies, correcting mechanics, redesigning feedback, and searching for training settings that could turn survival into completion. The seven-minute run is the cost of training one selected policy, not the cost of the project.

This article follows that process. The result is in our Fight Caves simulator, with an explicitly designed observation and action interface. It demonstrates repeatable completion from particular trained weights; later experiments also show how much the outcome can vary when we train those weights again from a different seed.
